...
s_filename = './csv/' + dt_day + '_crawl_twse_stockid.csv'

#本國上市證券國際證券辨識號碼一覽表
df1 = get_dataframe('http://isin.twse.com.tw/isin/C_public.jsp?strMode=2')
#本國上櫃證券國際證券辨識號碼一覽表
df2 = get_dataframe('http://isin.twse.com.tw/isin/C_public.jsp?strMode=4')
frames = [df1,df2]
df = pd.concat(frames)
#df.to_csv('./crawl_twse_stockid', sep=',', encoding='utf-8-sig', index=False)
